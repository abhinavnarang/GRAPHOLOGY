...
def extractLines(img):
	...
	'''
	# Print(ng the values we found so far.)
...
def extractWords(image, lines):
	...
	relative_word_spacing = average_word_spacing / (LETTER_SIZE + 0.1)
  ...
  # thresh = img
	...
	for i, angle in enumerate(theta):
		...
		for j, word in enumerate(words):
			original = img[word[0]:word[1], word[2]:word[3]] # y1:y2, x1:x2
	...
	if(max_index == 0):
		...
		p = s_function[4] / (s_function[3]+0.1)
		q = s_function[4] / (s_function[5]+0.1)
...
def start(file_name):
	...
	path = '/content/web/static/uploads/{}'.format(file_name)
  ...
  # print("wjbhlblbbl")
	...
	lineIndices = extractLines(image)
	#print(lineIndices)
	#print(	)
	...
	wordCoordinates = extractWords(image, lineIndices)
	...
	extractSlant(image, wordCoordinates)
